...
class Evaluator:
    ...
    def load(self, name):
        ...
        handle = open(os.path.join(self.path, name), 'rb')
    ...
    def train(self):
       ...
       # quit()
        ...
        for i in range(1, nb_epoch+1):
            ...
            #print(good_answers)
            test = len(good_answers)/2
            #bad_answers = self.pada(random.sample(list(self.answers.values()), 50 ))
            bad_answers = self.pada(random.sample(self.answers.values(), len(good_answers)))
